...
func checkArguments() (string, string, string) {
	// Expect 3 arguments: publicKey, signature, messageFilename
	if len(os.Args) != 4 {
		printUsage()
		os.Exit(1)
	}
...
// Return bool whether signature was verified
...
func loadPublicKeyFromPEM(publicKeyFilename string) *rsa.PublicKey {
	...
	if block == nil || block.Type != "PUBLIC KEY" {
		log.Fatal("Unable tp load valid public key")
	}
...
func main() {
	...
	publicKeyFilename, signatureFilename, messageFilename := checkArguments()

	// Load all the files from disk
	publicKey := loadPublicKeyFromPEM(publicKeyFilename)
