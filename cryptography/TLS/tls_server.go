...
func main() {
	...
	if err != nil {
		log.Fatalf("Error loading certificate and private key: %v", err)
	}
	...
	for {
		...
		if err != nil {
			log.Println("Error accepting client connection: %v", err)
			continue
		}
...
func handleConnection(clientConnection net.Conn) {
	...
	for {
		...
		if err != nil {
			log.Printf("Error writing data to client socket: %v", err)
			return
		}
